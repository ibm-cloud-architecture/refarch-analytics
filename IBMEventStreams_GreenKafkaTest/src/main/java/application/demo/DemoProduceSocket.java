...
public class DemoProduceSocket {
    ...
    private class DemoController implements Runnable {
        ...
        private RecordData produceMessage() {
            logger.debug(String.format("Producing message with custom content %s", customContent));
            DemoProducedMessage demoMessage = new DemoProducedMessage(customContent);
            RecordData recordData;
            try {
                RecordMetadata recordMetadata = producer.produce(demoMessage.toString());
